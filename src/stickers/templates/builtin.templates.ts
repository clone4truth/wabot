...
export class TerminalTemplate implements StickerTemplate {
  ...
  async render(input: { text?: string }): Promise<StickerResult> {
    ...
    const maxChars = 28;
    const lines = wrapWords(clean, maxChars, 8);

    const codeTexts = lines
      .map((line, idx) => {
        const y = 140 + idx * 36;
        const prefix = idx === 0 ? '<tspan fill="#48bb78">user@wabot:~$ </tspan>' : '<tspan fill="#718096">&gt; </tspan>';
        return `<text x="40" y="${y}" font-family="monospace,${family}" font-size="22" font-weight="bold" fill="#f7fafc">${prefix}${escapeXml(line)}</text>`;
      })
      .join('');
...
export class BreakingTemplate implements StickerTemplate {
  ...
  async render(input: { text?: string }): Promise<StickerResult> {
    ...
    const maxChars = 24;
    const lines = wrapWords(clean, maxChars, 6);

    const bodyTexts = lines
      .map((line, idx) => {
        const y = 180 + idx * 44;
        return `<text x="256" y="${y}" text-anchor="middle" font-family="${family},sans-serif" font-size="32" font-weight="bold" fill="#ffffff" stroke="#000000" stroke-width="2" paint-order="stroke">${escapeXml(line)}</text>`;
      })
      .join('');
...
export class WantedTemplate implements StickerTemplate {
  ...
  async render(input: { text?: string }): Promise<StickerResult> {
    ...
    const maxChars = 20;
    const lines = wrapWords(clean, maxChars, 5);

    const bodyTexts = lines
      .map((line, idx) => {
        const y = 220 + idx * 42;
        return `<text x="256" y="${y}" text-anchor="middle" font-family="${family},serif" font-size="34" font-weight="bold" fill="#3b2f2f">${escapeXml(line)}</text>`;
      })
      .join('');
...
export class MinimalTemplate implements StickerTemplate {
  ...
  async render(input: { text?: string }): Promise<StickerResult> {
    ...
    const bodyTexts = lines
      .map((line, idx) => {
        const y = 200 + idx * 46;
        return `<text x="256" y="${y}" text-anchor="middle" font-family="${family},sans-serif" font-size="34" font-weight="500" fill="#f8fafc">${escapeXml(line)}</text>`;
      })
      .join('');
